...
function SlotBlock({ slot, onToggle, onReschedule }: {
  slot: ScheduleSlot;
  ...
  return (
    <div className="schedule-slot">
      <div className="slot-time">{slot.start}</div>
      <div className={`slot-block ${isMissed ? 'slot-missed' : 'slot-task'}`}>
  ...
  return (
    <div>
      ...
      {missedCount > 0 && state.settings.autoReschedule && (
        <div className="reschedule-banner" role="alert">
          ...
          <div>
            ...
            <p>{missedCount} Aufgabe(n) nicht erledigt. Soll ich sie auf die nächsten Tage verteilen?</p>
          </div>
          <div style={{ marginLeft: 'auto' }}>
            <button
              className="btn btn-ghost btn-sm"
              onClick={() => dispatch({ type: 'AUTO_RESCHEDULE' })}
              aria-label="Automatisch umplanen"
            >
              Jetzt umplanen
            </button>
          </div>
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          <div className="stat-val">{mins}</div>
          <div className="stat-label">Min. fokussiert</div>
        </div>
        <div className="stat-card">
        ...
        <div className="stat-card">
          <div className="stat-val" style={{ color: 'var(--dopa)' }}>{state.streak}🔥</div>
          <div className="stat-label">Tages-Streak</div>
        </div>
      </div>
      ...
      {slots.length === 0 ? (
        ...
        <div className="schedule-day">
          <div className="schedule-day-header today">
            <Sun size={14} /> Heute
          </div>
          {slots.map((slot) => (
            <SlotBlock
